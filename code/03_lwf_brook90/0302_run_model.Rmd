...
meteo_path=paste(root,"data/meteo",sep="/")
...
meteo_era5=data.table(read.table(paste(meteo_path,"ERA5_meteo_daily.csv",sep="/"),header=T,sep=";",stringsAsFactors=F))
meteo_era5$dates=as.Date(meteo_era5$dates)

##adjust era5 data
# meteo_era5$prec=meteo_era5$prec*0.5
# meteo_era5$tmean=meteo_era5$tmean+5.0
# meteo_era5$tmin=meteo_era5$tmin+5.0
# meteo_era5$tmax=meteo_era5$tmax+5.0



##Walther-Lieth diagram
##aggregate to monthly values
tmin=setDT(meteo_era5)[year(dates)%in%year(start_date):year(end_date), .(tmin=min(tmin)),by=.(yr=year(dates),mon=month(dates))]
tmax=setDT(meteo_era5)[year(dates)%in%year(start_date):year(end_date), .(tmax=max(tmax)),by=.(yr=year(dates),mon=month(dates))]
psum=setDT(meteo_era5)[year(dates)%in%year(start_date):year(end_date), .(psum=sum(prec)),by=.(yr=year(dates),mon=month(dates))]
...
# hist(meteo_era5$prec)
# plot(meteo_era5$prec,type="h")
...
##import local meteo data
meteo_daily=data.table(read.table(paste(meteo_path,"meteo_daily_local.csv",sep="/"),header=T,sep=";",stringsAsFactors=F))
...
##double-mass plots
par(pty="s")
plot(meteo_era5$tmean,meteo_daily$tmean,asp=1,xlab="Tmean ERA5 [deg C]",ylab="Tmean local [deg C]")
abline(a=0,b=1,col="red",lwd=2)
grid()

plot(meteo_era5$tmin,meteo_daily$tmin,asp=1,xlab="Tmin ERA5 [deg C]",ylab="Tmin local [deg C]")
abline(a=0,b=1,col="red",lwd=2)
grid()

plot(meteo_era5$tmax,meteo_daily$tmax,asp=1,xlab="Tmax ERA5 [deg C]",ylab="Tmax local [deg C]")
abline(a=0,b=1,col="red",lwd=2)
grid()

plot(meteo_era5$prec,meteo_daily$prec,asp=1,xlab="Prec sum ERA5 [mm/d]",ylab="Prec sum local [mm/d]")
abline(a=0,b=1,col="red",lwd=2)
grid()

plot(meteo_era5$globrad,meteo_daily$globrad,asp=1,xlab="Globrad ERA5 [MJ/m2]",ylab="Globrad local [MJ/m2]")
abline(a=0,b=1,col="red",lwd=2)
grid()

plot(meteo_era5$vappres,meteo_daily$vappres,asp=1,xlab="Vappres ERA5 [kPa]",ylab="Vappres local [kPa]")
abline(a=0,b=1,col="red",lwd=2)
grid()

plot(meteo_era5$wind,meteo_daily$wind,asp=1,xlab="Wind ERA5 [m/s]",ylab="Wind local [m/s]")
abline(a=0,b=1,col="red",lwd=2)
grid()
...
##grass
options_grass=set_optionsLWFB90(startdate=start_date,
                                enddate=end_date,
                                budburst_method="ETCCDI",
...
# good package: library(soiltexture)
...
##load parameterizations for spruce (Picea abies), beech (Fagus sylvatica), oak (Quercus robur) and pine (Pinus sylvestris) from other project
load(paste(model_path,"veg_param.rda",sep="/"))

# average height from liDAR extracted trees (n=6)
params_spruce$height=26 # m
params_pine$height=26 # m

##grass parameterization taken from Schmidt-Walter et al. (2020), Supplement
params_grass=set_paramLWFB90(maxlai=3,
                             winlaifrac=0.5,
                             height=0.25,
                             sai=0,
                             glmax=0.0053,
...
ilayer=3 ##Number of layers from top to which infiltration is distributed
...
params_grass$coords_x=location$lon
params_grass$coords_y=location$lat
...
params_grass$aspect=location$aspect
params_grass$dslope=location$slope
params_grass$eslope=location$slope
...
##set location's drainage parameter (1 = free drainage, 0 = impervious lower boundary)
params_pine$drain=drain
params_spruce$drain=drain
params_grass$drain=drain
...
res_grass=run_LWFB90(options_b90=options_grass,
                      param_b90=params_grass,
                      climate=meteo_daily,
                      soil=soil
                      )
...
wb_grass=rbindlist(list(`grass`=res_grass$output),
                      idcol="vegetation")
...
wb_grass$date=as.Date(sprintf("%i-%0.2i-%0.2i",wb_grass$yr,wb_grass$mo,wb_grass$da),format="%Y-%m-%d")

##add surface runoff
wb_pine[,surfrunoff := flow-vrfln]
wb_spruce[,surfrunoff := flow-vrfln]
wb_grass[,surfrunoff := flow-vrfln]

##bind results
wb=rbind(wb_pine,wb_spruce,wb_grass)
...
nprec=xts(wb_spruce$rfal,order.by=wb_spruce$date)
sfal=xts(wb_spruce$sfal,order.by=wb_spruce$date)
smlt=xts(-wb_spruce$smlt,order.by=wb_spruce$date)
srf=xts(-wb_spruce$surfrunoff,order.by=wb_spruce$date)
...
##indices grass
output_grass$tratio=output_grass$tran/output_grass$ptran
output_grass$tratio[output_grass$tratio>1]=1
output_grass$tratio[is.nan(output_grass$tratio)]=1
output_grass$tratio[is.infinite(output_grass$tratio)]=1
output_grass$tdef=output_grass$ptran-output_grass$tran
output_grass$tdef[output_grass$tdef<0]=0

# ##calc indices for mixed forest
# mixed=data.frame(doy=output_spruce$doy,
#                  yr=output_spruce$yr,
#                  date=output_spruce$date,
#                  rnet=apply(cbind(output_spruce$rnet,output_pine$rnet),1,FUN="mean",na.rm=T),
#                  smlt=apply(cbind(output_spruce$smlt,output_pine$smlt),1,FUN="mean",na.rm=T),
#                  relawat=apply(cbind(output_spruce$relawat,output_pine$relawat),1,FUN="mean",na.rm=T),
#                  tdef=apply(cbind(output_spruce$tdef,output_pine$tdef),1,FUN="mean",na.rm=T),
#                  tratio=apply(cbind(output_spruce$tratio,output_pine$tratio),1,FUN="mean",na.rm=T)
# )

##calc indices for mixed forest with grass understory
mixed=data.frame(doy=output_spruce$doy,
                 yr=output_spruce$yr,
                 date=output_spruce$date,
                 rnet=apply(cbind(output_spruce$rnet,output_pine$rnet,output_grass$rnet),1,FUN="mean",na.rm=T),
                 smlt=apply(cbind(output_spruce$smlt,output_pine$smlt,output_grass$smlt),1,FUN="mean",na.rm=T),
                 relawat=apply(cbind(output_spruce$relawat,output_pine$relawat,output_grass$relawat),1,FUN="mean",na.rm=T),
                 tdef=apply(cbind(output_spruce$tdef,output_pine$tdef,output_grass$tdef),1,FUN="mean",na.rm=T),
                 tratio=apply(cbind(output_spruce$tratio,output_pine$tratio,output_grass$tratio),1,FUN="mean",na.rm=T)
)
...
col_grass="#ff916e"
col_mixed="#a387ff"
cols_treesp=c(col_spruce,col_pine,col_grass,col_mixed)

##prec and snow melt
prec=xts(output_spruce$rnet[output_spruce$yr%in%unique(output_spruce$yr)],order.by=output_spruce$date[output_spruce$yr%in%unique(output_spruce$yr)])
smlt=xts(output_spruce$smlt[output_spruce$yr%in%unique(output_spruce$yr)],order.by=output_spruce$date[output_spruce$yr%in%unique(output_spruce$yr)])
...
swat_grass=rbindlist(list(`grass`=res_grass$layer_output),
                   idcol="vegetation")
...
swat_grass[,dates:=as.POSIXct(paste(yr,mo,da,sep="-"),tz="UTC")]

##get theta at topmost mineral soil layer
soil_layer=2 ##select soil layer
swat_pine_nl=swat_pine[which(nl==soil_layer),list(theta=mean(theta)),by=dates]
swat_spruce_nl=swat_spruce[which(nl==soil_layer),list(theta=mean(theta)),by=dates]
swat_grass_nl=swat_grass[which(nl==soil_layer),list(theta=mean(theta)),by=dates]
...
ts_swat_grass_nl=xts(swat_grass_nl$theta*100,order.by=swat_grass_nl$dates)
tseries_plot_nl=merge(prec,smlt,ts_swat_spruce_nl,ts_swat_pine_nl,ts_swat_grass_nl)

##fill missing 15min measurements with closest measurement
#tseries_smc_filled=na.locf(tseries_plot,fromLast=F,maxgap=10)

##plot
cols_smc=c("#ff916e","#98FF85","#159837","#0800ff","#50ffff")
par(mar=c(5,5,5,5))
dygraph(tseries_plot_nl,width="auto",height=400) %>%
  dyAxis("y",label="Soil moisture content [%]",valueRange=c(10,60)) %>%
  ...
  dyOptions(stackedGraph=T,colors=cols_smc) %>%
  dySeries('ts_swat_spruce_nl',label="SMC spruce nl [%]",axis="y") %>%
  dySeries('ts_swat_pine_nl',label="SMC pine nl [%]",axis="y") %>%
  dySeries('ts_swat_grass_nl',label="SMC grass nl [%]",axis="y") %>%
  dyRangeSelector(height=20) %>%
  dyOptions(stackedGraph=F,colors=cols_smc) %>%
  dyLegend()
...
#year_plot=unique(output_spruce$yr)[1]
for (year_plot in unique(output_spruce$yr)){
  spruce_sub=output_spruce[output_spruce$yr==year_plot,]
  pine_sub=output_pine[output_pine$yr==year_plot,]
  grass_sub=output_grass[output_grass$yr==year_plot,]
  ...
  lines(grass_sub$doy,grass_sub$relawat,col=col_grass)
  ...
  points(spruce_sub$doy[spruce_sub$relawat<quantile(na.omit(spruce_sub$relawat),0.01)],
         spruce_sub$relawat[spruce_sub$relawat<quantile(na.omit(spruce_sub$relawat),0.01)],col=col_spruce,pch=1)
  ...
  points(grass_sub$doy[grass_sub$relawat<quantile(na.omit(grass_sub$relawat),0.01)],
         grass_sub$relawat[grass_sub$relawat<quantile(na.omit(grass_sub$relawat),0.01)],col=col_grass,pch=3)
  ...
  legend("topright",legend=c("Spruce","Pine","Mixed"),lwd=2,pch=c(1,3,5),col=cols_treesp)
...
##rfal and smlt from model results
Spruce=xts(output_spruce$relawat,order.by=output_spruce$date)
Pine=xts(output_pine$relawat[output_pine$yr%in%unique(output_spruce$yr)],order.by=output_pine$date[output_pine$yr%in%unique(output_spruce$yr)])
Grass=xts(output_grass$relawat[output_grass$yr%in%unique(output_spruce$yr)],order.by=output_grass$date[output_grass$yr%in%unique(output_spruce$yr)])
Mixed=xts(mixed$relawat[mixed$yr%in%unique(output_spruce$yr)],order.by=mixed$date[mixed$yr%in%unique(output_spruce$yr)])

tseries_plot=merge(prec,smlt,Spruce,Pine,Grass,Mixed)
...
dygraphs::dygraph(tseries_plot,width=600,height=400) %>%
  ...
  dyGroup(c("Spruce","Pine","Grass","Mixed"),drawPoints=F,color=cols_treesp,axis="y") %>%
...
Values \>Q99 are shown as point markers
...
for (year_plot in unique(output_spruce$yr)){
  ...
  lines(grass_sub$doy,grass_sub$tdef,col=col_grass)
  lines(mixed_sub$doy,mixed_sub$tdef,col=col_mixed)

  points(spruce_sub$doy[spruce_sub$tdef>quantile(na.omit(spruce_sub$tdef),0.99)],
         spruce_sub$tdef[spruce_sub$tdef>quantile(na.omit(spruce_sub$tdef),0.99)],col=col_spruce,pch=1)
  points(pine_sub$doy[pine_sub$tdef>quantile(na.omit(pine_sub$tdef),0.99)],
         pine_sub$tdef[pine_sub$tdef>quantile(na.omit(pine_sub$tdef),0.99)],col=col_pine,pch=3)
  points(grass_sub$doy[grass_sub$tdef>quantile(na.omit(grass_sub$tdef),0.99)],
         grass_sub$tdef[grass_sub$tdef>quantile(na.omit(grass_sub$tdef),0.99)],col=col_grass,pch=3)
  points(mixed_sub$doy[mixed_sub$tdef>quantile(na.omit(mixed_sub$tdef),0.99)],
         mixed_sub$tdef[mixed_sub$tdef>quantile(na.omit(mixed_sub$tdef),0.99)],col=col_mixed,pch=5)
  
  legend("bottomright",legend=c("Spruce","Pine","Grass","Mixed"),lwd=2,pch=c(1,3,5),col=cols_treesp)
}


##dygraph
##rfal and smlt from model results
Spruce=xts(output_spruce$tdef,order.by=output_spruce$date)
Pine=xts(output_pine$tdef[output_pine$yr%in%unique(output_spruce$yr)],order.by=output_pine$date[output_pine$yr%in%unique(output_spruce$yr)])
Grass=xts(output_grass$tdef[output_grass$yr%in%unique(output_spruce$yr)],order.by=output_grass$date[output_grass$yr%in%unique(output_spruce$yr)])
Mixed=xts(mixed$tdef[mixed$yr%in%unique(output_spruce$yr)],order.by=mixed$date[mixed$yr%in%unique(output_spruce$yr)])
...
for (year_plot in unique(output_spruce$yr)){
  ...
  lines(grass_sub$doy,grass_sub$tratio,col=col_grass)
  ...
  points(spruce_sub$doy[spruce_sub$tratio<quantile(na.omit(spruce_sub$tratio),0.01)],
         spruce_sub$tratio[spruce_sub$tratio<quantile(na.omit(spruce_sub$tratio),0.01)],col=col_spruce,pch=1)
  ...
  points(grass_sub$doy[grass_sub$tratio<quantile(na.omit(grass_sub$tratio),0.01)],
         grass_sub$tratio[grass_sub$tratio<quantile(na.omit(grass_sub$tratio),0.01)],col=col_grass,pch=3)
...
##dygraph
##rfal and smlt from model results
Spruce=xts(output_spruce$tratio,order.by=output_spruce$date)
Pine=xts(output_pine$tratio[output_pine$yr%in%unique(output_spruce$yr)],order.by=output_pine$date[output_pine$yr%in%unique(output_spruce$yr)])
Grass=xts(output_grass$tratio[output_grass$yr%in%unique(output_spruce$yr)],order.by=output_grass$date[output_grass$yr%in%unique(output_spruce$yr)])
Mixed=xts(mixed$tratio[mixed$yr%in%unique(output_spruce$yr)],order.by=mixed$date[mixed$yr%in%unique(output_spruce$yr)])
...
lines(output_grass$date,output_grass$relawat,col=col_grass)
...
lines(output_grass$date,output_grass$tdef,col=col_grass)
lines(mixed_sub$doy,mixed_sub$tdef,col=col_mixed)
...
lines(output_grass$date,output_grass$tratio,col=col_grass)
lines(mixed_sub$doy,mixed_sub$tratio,col=col_mixed)
legend("bottomright",legend=c("Spruce","Pine","Grass","Mixed"),lwd=2,col=cols_treesp)
...
write.table(output_grass[,c("date","tratio","tdef","relawat")],paste(model_path,sprintf("wb_indices_%s_grass.csv",pid),sep="/"),sep=";",quote=F,row.names=F)

write.table(mixed[,c("date","tratio","tdef","relawat")],paste(model_path,sprintf("wb_indices_%s_mixed.csv",pid),sep="/"),sep=";",quote=F,row.names=F)
