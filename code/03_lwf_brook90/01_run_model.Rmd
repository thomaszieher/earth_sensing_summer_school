...
# ##add humus layer
# hum_depth=-0.1 #m
# hum_layer=data.table(lower=hum_depth,hor_num=0,gravel=NA,corg=NA,topsoil=NA,bd=NA,upper=0,dw=NA,clay=NA,silt=NA,sand=NA,
...
swat_spruce_nl1=swat_spruce[which(nl==1),list(theta=mean(theta)),by=dates]
swat_beech_nl1=swat_beech[which(nl==1),list(theta=mean(theta)),by=dates]

##compare
ts_swat_spruce_nl1=xts(swat_spruce_nl1$theta*100,order.by=swat_spruce_nl1$dates)
ts_swat_beech_nl1=xts(swat_beech_nl1$theta*100,order.by=swat_beech_nl1$dates)
tseries_plot_5cm=merge(prec,smlt,ts_swat_beech_nl1,ts_swat_spruce_nl1)
...
par(mar=c(5,5,5,5))
dygraph(tseries_plot_5cm,width="auto",height=400) %>%
  ...
  dySeries('ts_swat_beech_nl1',label="SMC beech nl1 [%]",axis="y") %>%
  dySeries('ts_swat_spruce_nl1',label="SMC spruce nl1 [%]",axis="y") %>%
  dyRangeSelector(height=20) %>%
  dyOptions(stackedGraph=F,colors=cols_prec) %>%
  dyLegend()
